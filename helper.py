...
# This is NLTK's English stop-word list, kept in the source so predictions do
# not rely on a separate, easily-missed ``stopwords.pkl`` runtime artifact.
STOP_WORDS = frozenset({
    ...
    'them', 'themselves', 'then', 'there', 'these', 'they', "they're", 'this',
...
def query_point_creator(q1, q2):
    ...
    # bow features for q1 and q2
    if cv is not None and hasattr(cv, 'transform'):
        q1_bow = cv.transform([q1]).toarray()
        q2_bow = cv.transform([q2]).toarray()
    else:
        # Fallback to zero vectors of 3000 dimensions matching model's expected 6022 features
        q1_bow = np.zeros((1, 3000))
        q2_bow = np.zeros((1, 3000))
